...
public class Lista {
    ...
    /**
     * devuelve la cantidad de elementos de la lista que fue actualizada en el
     * insertar y eliminar
     */
    public int longitud() {
    ...
    public Lista clone() {
        ...
        if (!esVacia()) {
            ...
            while (actual.getEnlace() != null) {
                aux.setEnlace(new Nodo(actual.getEnlace().getElemento(), null));
                aux = aux.getEnlace();
    ...
    public void agregarElementos(Lista clon) {
        if (!clon.esVacia()) {
            Nodo actual = clon.cabecera;
            for (int index = 1; index <= clon.longitud; index++) {
                this.insertar(actual.getElemento(), index);
                actual = actual.getEnlace();
            }
        }
    }
    ...
    public boolean pertenece(Object busc) {
        boolean pertenece = false;
        if (!this.esVacia()) {
            pertenece = pertenecePasoRecursivo(this.cabecera, busc);
        }
        return pertenece;
    }

    private boolean pertenecePasoRecursivo(Nodo actual, Object elemento) {
        boolean pertenece = false;
        if (actual.getElemento().equals(elemento)) {
            pertenece = true;
        } else {
            if (actual.getEnlace() != null) {
                pertenece = pertenecePasoRecursivo(actual.getEnlace(), elemento);
            }
        }
        return pertenece;
    }
